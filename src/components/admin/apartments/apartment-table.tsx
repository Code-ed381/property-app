...
import { columns, Apartment } from "./apartment-columns";

interface ApartmentTableProps {
  data: Apartment[];
}

export function ApartmentTable({ data }: ApartmentTableProps) {
    ...
    <div className="w-full space-y-4">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" className="ml-auto border-slate-800 bg-slate-900/50 hover:bg-slate-900 text-slate-300 transition-all rounded-xl">
                <SlidersHorizontal className="mr-2 h-4 w-4" /> Views
              </Button>
            </DropdownMenuTrigger>
      ...
      <div className="flex items-center justify-between px-2">
        <div className="text-xs text-slate-500 font-medium">
          Showing {table.getFilteredRowModel().rows.length} apartments
        </div>
