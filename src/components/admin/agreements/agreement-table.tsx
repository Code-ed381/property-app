...
import { columns, Agreement } from "./agreement-columns";
    ...
    <div className="w-full space-y-4">
      <div className="flex flex-col md:flex-row items-center gap-4 justify-between">
        <div className="relative w-full md:max-w-sm group">
          ...
          <Input
            ...
            value={(table.getColumn("tenant_email")?.getFilterValue() as string) ?? ""}
            onChange={(event) =>
              table.getColumn("tenant_email")?.setFilterValue(event.target.value)
            }
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" className="ml-auto border-slate-800 bg-slate-900/50 hover:bg-slate-900 text-slate-300 transition-all rounded-xl">
                <SlidersHorizontal className="mr-2 h-4 w-4" /> Layout
              </Button>
            </DropdownMenuTrigger>
      ...
      <div className="flex items-center justify-between px-2">
        <div className="text-xs text-slate-500 font-medium">
          Showing {table.getFilteredRowModel().rows.length} agreements
        </div>
